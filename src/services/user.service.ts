...
export const UserService = {
  getUserInfo: async function (key: string, value: string) {
    const response = await axiosInstance.get<UserAccounts>(API_ESOURECES.INSPECT_USER, {
      params: {
        [key]: value,
      },
    })
  ...
  changeServices: async function (email: string, input: ServicesInput) {
    return axiosInstance.patch(`${API_ESOURECES.SERVICES_EDIT}?email=${email}`, input)
  },
